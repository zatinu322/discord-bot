...
from bot.events import get_reaction_role_rules, register_events
from bot.helpers import get_drunk_members, schedule_sobriety, set_role
...
class Bot(commands.Bot):
    def __init__(self, config: "DiscordSettings", **kwargs):
        ...
        self.reaction_role_rules = get_reaction_role_rules(config)
    ...
    async def on_ready(self) -> None:
        ...
        self.entrance_role = self.guild.get_role(self.config.ENTRANCE.ROLE_ID)
        self.vip_role = self.guild.get_role(self.config.VIP_ROLE_ID)
    ...
    async def ensure_reaction_roles(self) -> None:
        ...
        for rule in self.reaction_role_rules:
            ...
            for user_id in expected_users - current_users:
                ...
                await set_role(self, user_id, rule.role_id, rule.restricted_roles)
    ...
    async def schedule_unmutes(self) -> None:
        ...
        for drunkard in drunk_members:
            ...
            tasks.append(
                    schedule_sobriety(
                    user=member,
                    sober_role=self.drunk_role,
                    mute_seconds=seconds_left,
                ),
            )
            logger.info(
                'Scheduled %s sobriety at %s.',
                member.display_name,
